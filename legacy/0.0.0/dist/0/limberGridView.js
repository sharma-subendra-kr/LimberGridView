...
window.LimberGridView = (function(){
	...
	*/
	...
	LimberGridView.prototype.renderItems = function(items, scale = true, processType = "onDemand"){
		...
		var renderDetails = {"items" : JSON.parse(JSON.stringify(items)), "scale" : scale, "processType" : processType};
	...
	LimberGridView.prototype.resizePlane = function(index, width, height){
		...
		item.width = width;
		item.height = height;
		var freeSpaces = this.findFreeSpaces(item, allAffectedItems.affectedItems);
	...
	LimberGridView.prototype.findAffectedItemsOnMove = function(index, toX, toY){
		...
		return {"affectedItems" : affectedItems, "affectedItemsExceptSelf" : affectedItemsExceptSelf};
	};
	...
	LimberGridView.prototype.isPlaneBInsidePlaneA_TouchingIsInside = function(A, B, resultType){
		...
		for(var i = 0; i < length_0; i++){
			if(BCoordinatesArr[i][0] >= ATopLeft[0] && BCoordinatesArr[i][0] <= ATopRight[0]
				&& BCoordinatesArr[i][0] >= ABottomLeft[0] && BCoordinatesArr[i][0] <= ABottomRight[0]
				&& BCoordinatesArr[i][1] >= ATopLeft[1] && BCoordinatesArr[i][1] <=ABottomLeft[1]
				&& BCoordinatesArr[i][1] >= ATopRight[1] && BCoordinatesArr[i][1] <=ABottomRight[1]
				){
				BResultArr[i] = true;
				isInside++;
				overallResult = true;
		...
		for(var i = 0; i < length_0; i++){
			if(ACoordiantesArr[i][0] >= BTopLeft[0] && ACoordiantesArr[i][0] <= BTopRight[0]
				&& ACoordiantesArr[i][0] >= BBottomLeft[0] && ACoordiantesArr[i][0] <= BBottomRight[0]
				&& ACoordiantesArr[i][1] >= BTopLeft[1] && ACoordiantesArr[i][1] <=BBottomLeft[1]
				&& ACoordiantesArr[i][1] >= BTopRight[1] && ACoordiantesArr[i][1] <=BBottomRight[1]
				){
				overallResult = true;
				engulfedCheckCount++;
			}
		...
		if((BTopLeft[0] > ATopLeft[0] && BTopRight[0] < ATopRight[0]
			&& BTopLeft[1] < ATopLeft[1] && BBottomLeft[1] > ABottomLeft[1])
			||
			(BTopLeft[1] > ATopLeft[1] && BBottomLeft[1] < ABottomLeft[1]
			&& BTopLeft[0] < ATopLeft[0] && BTopRight[0] > ATopRight[0])
			){
			overallResult = true;
		}
		...
		if(resultType == "overall"){
			...
			return {overall : overallResult, individual : BResultArr, isInside : isInside, engulfed : engulfed};
		}else{
			return overallResult;
		}
	...
	LimberGridView.prototype.isPlaneBInsidePlaneA_TouchingNotInside = function(A, B, resultType){
		...
		if((ATopLeft[0] >= BTopLeft[0] && ATopRight[0] <= BTopRight[0]
			&& ATopLeft[1] <= BTopLeft[1] && ABottomLeft[1] >= BBottomLeft[1])
			||
			(ATopLeft[1] >= BTopLeft[1] && ABottomLeft[1] <= BBottomLeft[1]
			&& ATopLeft[0] <= BTopLeft[0] && ATopRight[0] >= BTopRight[0])
			){
			overallResult = true;
		}
		...
		if((BTopLeft[1] == ATopLeft[1] && BBottomLeft[1] == ABottomLeft[1])
	...
	LimberGridView.prototype.findMatchingSpace = function(items, freeSpaces){
		...
		return {"matchingItemIndex" : matchingItemIndex, "matchingFreeSpaceIndex" :matchingFreeSpaceIndex};
	};
	...
	LimberGridView.prototype.fitRemainingItems = function(positionData, remainingItems, allAffectedItems, changedIndex, previousFitDetails){
		...
		for(var i = 0; i < length_0; i++){
			...
			if((positionData[i].y < greatestDepthAffectedItems) 
	...
	LimberGridView.prototype.fitRemainingItemsBelowDeepestLine = function(startingY, positionData, remainingItems, allAffectedItems){
		...
		var initializedPlane = {x : 0, y : startingY, width : this.WIDTH, height : remainingItemsObject[0].height + (this.getMarginAtPoint(startingY))};
	...
	LimberGridView.prototype.getItemsFromPointDepth = function(startingY, includePlaneAboveAndBelowPointDepth = false, positionData){
		...
		return {"itemsFromPointDepth" : itemsFromPointDepth, "shiftHeightToAdd" : shiftHeightToAdd};
	};
		...
		if(result.overall == true){
			if(A.x <= B.x){
				if((A.x + A.width) > (B.x)){
					...
					if(A.y < B.y){
						var plane = {x : A.x, y :A.y, width : A.width, height : B.y - A.y};
						if(this.isValidPlane(plane)){
							planes.push(plane);
						}
						if((A.y+A.height) < (B.y+B.height)){
							var plane = {x : A.x, y : B.y, width : B.x - A.x, height : (A.y + A.height) - B.y};
							if(this.isValidPlane(plane)){
								planes.push(plane);
							}
						}else if((A.y+A.height) >= (B.y+B.height)){
							var plane = {x : A.x, y : B.y, width : B.x - A.x, height : B.height};
							if(this.isValidPlane(plane)){
								planes.push(plane);
							}
							if((A.y+A.height) > (B.y+B.height)){
								var plane = {x : A.x, y : (B.y + B.height), width : A.width, height : (A.y + A.height) - (B.y + B.height)};
								if(this.isValidPlane(plane)){
									planes.push(plane);
								}
							}
						...
						if((A.y + A.height) < (B.y + B.height)){
							var plane = {x : A.x, y : A.y, width : B.x - A.x, height : A.height};
							if(this.isValidPlane(plane)){
								planes.push(plane);
							}
						}else{
							var plane = {x : A.x, y : A.y, width : B.x - A.x, height : (B.y + B.height) - A.y};
							if(this.isValidPlane(plane)){
								planes.push(plane);
							}
						}
				...
				if((A.x + A.width) > (B.x + B.width)){
					if(A.y < B.y){
						if((A.y + A.height) < (B.y + B.height)){
							var plane = {x : B.x + B.width, y : B.y, width : (A.x + A.width) - (B.x + B.width), height : (A.y + A.height) - B.y};
							if(this.isValidPlane(plane)){
								planes.push(plane);
							}
						}else{
							var plane = {x : B.x + B.width, y : B.y, width : (A.x + A.width) - (B.x + B.width), height : B.height};
							if(this.isValidPlane(plane)){
								planes.push(plane);
							}
						}	
						...
						if((A.y + A.height) < (B.y + B.height)){
							var plane = {x : B.x + B.width, y : A.y, width : (A.x + A.width) - (B.x + B.width), height : A.height};
							if(this.isValidPlane(plane)){
								planes.push(plane);
							}
						}else{
							var plane = {x : B.x + B.width, y : A.y, width : (A.x + A.width) - (B.x + B.width), height : (B.y + B.height) - A.y};
							if(this.isValidPlane(plane)){
								planes.push(plane);
							}
						}
	...
	LimberGridView.prototype.onLimberGridMouseDownCheck = function(event, offsetX, offsetY){
		if(this.limberGridMouseDownCancel == false){
			...
			this.userActionData = { type : "add", addPositionX : x, addPositionY : y};
	...
	LimberGridView.prototype.onItemMouseDown = function(event){
		...
		if(radius <= this.MOVE_GUIDE_RADIUS){
			this.userActionData = { type : "move", itemIndex : event.currentTarget.attributes["data-index"].value};
			...
			event.preventDefault();
		}else if(event.offsetX >= resizeUIBox.x && event.offsetX <= (resizeUIBox.x + resizeUIBox.width)
			&& event.offsetY >= resizeUIBox.y && event.offsetY <= (resizeUIBox.y + resizeUIBox.height)
		){
			this.$limberGridViewHeightAdjustGuide[0].style.height = (0) + "px";
			this.$limberGridViewHeightAdjustGuide[0].classList.add("limberGridViewHeightAdjustGuideActive");

			this.userActionData = { type : "resize", itemIndex : event.currentTarget.attributes["data-index"].value};
			this.mouseDownCancel = false;
	...
	LimberGridView.prototype.onItemTouchStart = function(event) {
		...
		if(radius <= this.MOVE_GUIDE_RADIUS){
			...
			event.preventDefault();
		}else if(touchPosOnLimberGridItem.x >= resizeUIBox.x && touchPosOnLimberGridItem.x <= (resizeUIBox.x + resizeUIBox.width)
			&& touchPosOnLimberGridItem.y >= resizeUIBox.y && touchPosOnLimberGridItem.y <= (resizeUIBox.y + resizeUIBox.height)
		){
			this.$limberGridViewHeightAdjustGuide[0].style.height = (0) + "px";
			this.$limberGridViewHeightAdjustGuide[0].classList.add("limberGridViewHeightAdjustGuideActive");
	...
	LimberGridView.prototype.mouseDownCheck = function(event){
		if(this.mouseDownCancel == false){
			...
			this.$limberGridViewBodyPseudoItems[this.userActionData.itemIndex].style.transform = "translate(" + (event.pageX /*+ 5*/) + "px, " + (event.pageY /*+ 5*/) + "px)";
	...
	LimberGridView.prototype.tapHoldCheck = function(event){
		if(this.tapHoldCancel == false){
			...
			this.$limberGridViewItems[this.userActionData.itemIndex].classList.add("limberGridViewItemDemo");
			this.$limberGridViewBodyPseudoItems[this.userActionData.itemIndex].classList.add("limberGridViewBodyPseudoItemActive");
			this.$limberGridViewBodyPseudoItems[this.userActionData.itemIndex].style.transform = "translate(" + (event.touches[0].pageX /*+ 5*/) + "px, " + (event.touches[0].pageY /*+ 5*/) + "px)";
	...
	LimberGridView.prototype.onMouseUp = function(event){
		...
		if(this.callbacks.resizeCompleteCallback != undefined && this.callbacks.resizeCompleteCallback != null){
			if(itemResizeFlag == true){			
				this.callbacks.resizeCompleteCallback(this.userActionData.itemIndex, {x : this.positionData[this.userActionData.itemIndex].x, y : this.positionData[this.userActionData.itemIndex].y, height : newHeight, width : newWidth});
			}
		}
		// 
	...
	LimberGridView.prototype.calculateMousePosOnLimberGrid = function(event){
		...
		if((event.clientX >= limberGridViewPosition.left && event.clientX <= (limberGridViewPosition.left + limberGridViewPosition.width))
			&& (event.clientY >= limberGridViewPosition.top && event.clientY <= (limberGridViewPosition.top + limberGridViewPosition.height))){
	...
	LimberGridView.prototype.calculateTouchPosOnLimberGrid = function(event){
		...
		if(event.type == "touchend"){
			var touch = {clientX : event.changedTouches[0].clientX, clientY : event.changedTouches[0].clientY};	
		}else{
			var touch = {clientX : event.touches[0].clientX, clientY : event.touches[0].clientY};
		}
		if((touch.clientX >= limberGridViewPosition.left && touch.clientX <= (limberGridViewPosition.left + limberGridViewPosition.width))
			&& (touch.clientY >= limberGridViewPosition.top && touch.clientY <= (limberGridViewPosition.top + limberGridViewPosition.height))){
	...
	LimberGridView.prototype.calculateTouchPosOnLimberGridItem = function(event){
		...
		if((event.touches[0].clientX >= limberGridViewItemPosition.left && event.touches[0].clientX <= (limberGridViewItemPosition.left + limberGridViewItemPosition.width))
			&& (event.touches[0].clientY >= limberGridViewItemPosition.top && event.touches[0].clientY <= (limberGridViewItemPosition.top + limberGridViewItemPosition.height))){
	...
	LimberGridView.prototype.checkNewMoveCoordinates = function(indexOfMovedItem, mousePositions){
		...
		for(var i = 0; i < length_0; i++){
			...
			if(x >= itemTopLeft[0] && x <= itemTopRight[0]
				&& x >= itemBottomLeft[0] && x <= itemBottomRight[0]
				&& y >= itemTopLeft[1] && y <=itemBottomLeft[1]
				&& y >= itemTopRight[1] && y <=itemBottomRight[1]
				){
				isInside = true;
			...
			if((y > bottomLine[0][1] && bottomLine[0][0] >= x && bottomLine[0][0] <= (x + widthOfMovedItem))
				|| ((y > bottomLine[0][1] && bottomLine[1][0] >= x && bottomLine[1][0] <= (x + widthOfMovedItem)))
			){
				var diff = y - bottomLine[0][1];
				if(diff <= this.MARGIN){
					return false;
			...
			if((x > rightLine[0][0] && rightLine[0][1] >= y && rightLine[0][1] <= (y + heightOfMovedItem))
				|| ((x > rightLine[0][0] && rightLine[1][1] >= y && rightLine[1][1] <= (y + heightOfMovedItem)))
			){
				var diff = x - rightLine[0][0];
				if(diff <= this.MARGIN){
					return false;
		...
		if(indexOfOverlappingItem ==  null){
			...
			if((this.positionData[indexOfOverlappingItem].x + this.positionData[indexOfMovedItem].width + this.getMarginAtPoint(this.positionData[indexOfOverlappingItem].x)) > this.WIDTH){
				...
				return {x : x, y : y, revisedCoordinates : {x : this.positionData[indexOfOverlappingItem].x, y : this.positionData[indexOfOverlappingItem].y}};
			}
		}
	};
